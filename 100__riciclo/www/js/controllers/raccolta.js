...
.controller('TDRCtrl', function ($scope, $rootScope, DataManager, Raccolta, Utili, $location) {
  $scope.icon = function(item) {
    return Utili.icon(item.tipologiaPuntoRaccolta,item.colore);
  };
    
    // !***!!!????%%%%$$"£"£!
    $scope.isPorta = function(item) {
        for(var key in item) {
            var val = item[key];
            if(Utili.isPaP(val.tipologiaPuntoRaccolta)) {
                return true;
            }
        }
        return false;
  ...
  $rootScope.$watch('selectedProfile',function(a,b) {
    ...
    //funzione al posto di href
    $scope.onclick = function(indirizzo) {
        $location.path(indirizzo);
